...
tasks=int(input('¿Cuántos procesamientos quieres hacer? '))
...
def saveheatmap (array3d, titlestring):
    ...
    if beforesave == 'y':
        ...
        while True: 
            isfirst=False
            ugib1 = int(input('Escribe valor mínimo'))
            ugib2 = int(input('Escriba valor máximo'))
    ...
    for i in directorylist:
        ...
        for i in meow:
            if not os.path.exists(i + '\\MTCarpeta'):
                ...
                qmt = AskUser('Pulsa y cuando esté listo y n para no hacer este procesamiento' )
                if qmt == 'y':
                    print('')
...
def ProcesarDTIs(filelist):
    ...
    counter = 1
    for n in printadc:
        saveheatmap(n, ('ADC dir' + str(counter)))
        counter += 1
...
for i in funky:
    if 'T2' in i:
        ...
        os.chdir((('\\').join(i.split("\\")[0:-1])) + '\\')
        getT2T2star.TxyFitME(i, ts_file, (('\\').join(i.split("\\")[0:-1]) + '\\'), algo, ncpu, ('\\').join(i.split("\\")[0:-1]) + '\\' + 'T2starmask.nii')
        if not os.path.exists('otrosarchivos'):
            os.makedirs('otrosarchivos')
        otherfiles = os.listdir((('\\').join(i.split("\\")[0:-1])))
        tomove = []
